...
OpenSLAudioPlay::OpenSLAudioPlay(int sampleRate, int sampleFormat, int channels) :
        mAudioEngine(new AudioEngine()), mPlayerObj(NULL), mPlayer(NULL), mBufferQueue(NULL),
        mEffectSend(NULL), mVolume(NULL), mSampleRate((SLmilliHertz) sampleRate * 1000),
...
// 一帧音频播放完毕后就会回调这个函数
void playerCallback(SLAndroidSimpleBufferQueueItf bq, void *context) {
    OpenSLAudioPlay *player = static_cast<OpenSLAudioPlay *>(context);
